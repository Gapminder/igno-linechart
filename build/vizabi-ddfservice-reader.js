...
var defaultParsers = {
  'YYYYMMDD': function YYYYMMDD(timeString) {
    return new Date(Date.UTC(timeString.slice(0, 4), timeString.slice(4, 6), timeString.slice(-2)));
  },
  'YYYYMM': function YYYYMM(timeString) {
    return new Date(Date.UTC(timeString.slice(0, 4), timeString.slice(-2)));
  },
  'YYYY': function YYYY(timeString) {
    return new Date(Date.UTC(timeString, 0));
  },
...
var getReader = function getReader() {
  ...
  return {
    ...
    parsers: {
      time: function time(timeString) {
        var timeRegEx = /^([0-9]{4})(w[0-9]{2}|q[0-9]{1})?([0-9]{2})?([0-9]{2})?$/;

        if (Number.isInteger(timeString)) {
          // the timeString is probably a year
          ...
          if (match) {
            // match[1] = year, match[2] = week or quarter, match[3] = month, match[4] = day
            if (match[4]) {
      ...
      year: function year(timeString) {
        return defaultParsers['YYYY'](timeString);
      },
      month: function month(timeString) {
        return defaultParsers['YYYYMM'](timeString);
      },
      day: function day(timeString) {
        return defaultParsers['YYYYMMDD'](timeString);
      },
      week: function week(timeString) {
        return defaultParsers['YYYYwWW'](timeString);
      },
      quarter: function quarter(timeString) {
        return defaultParsers['YYYYqQ'](timeString);
      }
    },
